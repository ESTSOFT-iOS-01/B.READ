...
protocol QuoteRepository {
  ...
  /// 기존 Quote를 갱신합니다.
  ///
  /// - Parameter quote: 업데이트할 `Quote` 엔티티 (같은 ID를 사용한 새로운 값)
  /// - Throws:
  ///   - `RepositoryError.dataNotFound`: 수정 대상이 존재하지 않는 경우
  ///   - `RepositoryError.fetchError`: 내부 조회 중 오류가 발생한 경우
  func updateQuote(_ quote: Quote) async throws
  
  /// 지정된 ID의 Quote를 삭제합니다.
  ///
  /// - Parameter id: 삭제할 `Quote` 엔티티의 ID
  /// - Throws:
  ...
  /// - Throws:
  ///   - `RepositoryError.fetchError`: 데이터 조회 중 오류가 발생한 경우
  func fetchQuotes(isbn: String) async throws -> [Quote]
  
  /// ID로 단일 Quote를 조회합니다.
  ///
  /// - Parameter id: 조회할 Quote의 ID
